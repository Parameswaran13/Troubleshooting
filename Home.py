...
api_key = 'AIzaSyAU2IHjNwbqvMIv9kt7oYQCdDhdTOQos50'
...
command2 = """ CREATE TABLE IF NOT EXISTS Video(\
    ...
    published_date DATETIME,duration int,\
...
with tab3:
    ...
    def insert_into_Channel(user_input):
        ...
        try:
            ...
            #data_types={'channel_name':'VARCHAR(255)',
            ...
            #channel_df.to_sql('Channel',connection,if_exists='replace',index=False,dtype=data_types)
    ...
    def insert_into_video(user_input):
        ...
        try:
            ...
            video_df = [i for i in collection2.find({},{'_id':0})]
            video_df=pd.DataFrame(video_df)
            video_df = video_df.drop_duplicates(subset=['video_id'],keep="first")

            video_df.to_sql('Video',connection,if_exists='replace',index=False,dtype=data_types)
    ...
    def insert_into_comment(user_input):
        collection3 = Mongodb.video_details
        collection2 = Mongodb.comment_details
        try:
            connection = sqlite3.connect('youtube1.db')
            cursor = connection.cursor()
            data_types={
            ...
            video_id = [i for i in collection3.find({'channel_name':user_input},{'_id':0})]
            comment_details = [i for i in collection2.find({},{'_id':0})]
            ...
            c_id= pd.DataFrame(comment_details)
            comment_details = c_id.drop_duplicates(subset=['video_id'],keep="first")
            v_id = pd.DataFrame(video_id)
            video_id = v_id.drop_duplicates(subset=['video_id'],keep="first")
            data = comment_details.merge(video_id,on='video_id',how='inner')
            data_df= data.reindex(columns=['comment_id','video_id','comment_text','comment_author','comment_posted_date','like_count','reply_count'])
            
            data_df.to_sql('Comment',connection,if_exists='replace',index=False,dtype=data_types)
