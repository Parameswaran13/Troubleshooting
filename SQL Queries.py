...
if option == '1. What are the names of all the videos and their corresponding channels?':
    ...
    connection = sqlite3.connect('youtube1.db')
    cur=connection.cursor()
    cur.execute("""SELECT channel_name, views FROM Channel ORDER BY views DESC""")
    ...
    SELECT channel_name, 
           CASE
               WHEN duration LIKE 'PT%S' THEN 
                   CAST(SUBSTR(duration, 3, LENGTH(duration) - 3) AS INTEGER)
               WHEN duration LIKE 'PT%M%S' THEN 
                   CAST(SUBSTR(duration, 3, LENGTH(duration) - 4) AS INTEGER)
               WHEN duration LIKE 'PT%H%M%S' THEN 
                   CAST(SUBSTR(duration, 3, LENGTH(duration) - 6) AS INTEGER)
           END AS duration_sec
    FROM Video) AS subquery
    GROUP BY channel_name
     """
    cur.execute(query)
    column_names = [description[0] for description in cur.description]
    df9 = pd.DataFrame(cur.fetchall(),columns=column_names)
